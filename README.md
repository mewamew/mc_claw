...
```
mc-claw/
├── SKILL.md                     # Agent 技能定义（OpenClaw / Claude Code 可直接导入）
├── bot-service/
...
本仓库同时是一个 **Agent Skill 包**：根目录的 `SKILL.md` 遵循 OpenClaw / Anthropic Agent Skills 规范，可以被直接导入到支持该规范的 Agent 平台（OpenClaw、Claude Code 等）。

## 作为技能包导入

### OpenClaw

1. 打开 OpenClaw → 设置 → 技能管理 → 添加技能 → 从 GitHub 导入
2. 填入本仓库 URL，例如 `https://github.com/<your-user>/mc_claw`
3. 导入成功后，技能会出现在"用户添加技能"列表中，启用即可

导入时 OpenClaw 会把整个仓库复制到 `~/.openclaw/skills/mc-claw/`，其中 `bot-service/` 也会一并被复制（无副作用，只是多占几 MB 空间）。

### Claude Code / 其他 Agent 框架

直接把 `SKILL.md` 内容作为 System Prompt 或知识库加载即可。它不依赖本仓库其他文件，只要 bot-service 在 `localhost:3001` 运行就能工作。
...
根目录的 `SKILL.md` 提供了完整的 API 使用指南和决策框架，可作为 Agent 的 System Prompt 或知识库使用。
